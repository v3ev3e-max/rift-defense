...
export function makeEquipment(templateId:string,rarity:HeroGrade,order=Date.now(),id=`eq-${order}-${Math.random().toString(36).slice(2,8)}`):EquipmentItem{
 ...
 return {id,templateId,name:t.name,slot:t.slot,weaponGroup:w?.weaponGroup,rarity,enhance:0,attack:w?Math.round((18+variant*2)*scale):0,hp:a?Math.round((110+(a.id==='armor-guardian'?35:0))*scale):0,speed:w&&variant===2?.04*scale:0,crit:w&&variant===1?.035*scale:0,skill:w&&variant===3?.07*scale:0,element:n?.element,elementDamage:n?({B:.05,A:.08,S:.11,SR:.15}[rarity]):0,effect:t.effect,equippedBy:undefined,locked:false,order};
}
