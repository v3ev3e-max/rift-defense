...
describe('Phase 16 contracts',()=>{
 ...
 it('continues past checkpoints and rotates all four bosses without invalid enemy IDs',()=>{
  ...
  const m=new BattleModel(defaultSave(),()=>.99);m.start();m.wave.start(31);m.wave.queue=[];m.step(1/60);expect(m.intermission).toBeCloseTo(.01);m.step(1/60);expect(m.wave.number).toBe(32);expect(m.ended).toBe(false);
 });
});
