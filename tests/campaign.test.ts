...
describe('campaign foundations',()=>{
 ...
 it('preserves region 16 clear records and migrates independent operation scores',()=>{const save=defaultSave() as any;save.campaign.records['16-10']={stars:3,time:91,kills:140};save.operations={riftBest:12345,constraintBest:9876,hardBest:7777,lastDaily:'2026-09-26'};const migrated=parseSave(JSON.stringify(save));expect(migrated.campaign?.records['16-10']).toEqual({stars:3,time:91,kills:140});expect(migrated.operations).toEqual(save.operations);});
 ...
 it.each(campaignStages)('simulates default squad through $id',async(stage)=>{
   {const save=defaultSave();for(const hero of Object.values(save.heroes)){hero.equipment=[];hero.stars=campaignRegionBalance[Number(stage.id.split('-')[0])-1].stars;}const m=new BattleModel(save,()=>.5);m.configureCampaign(stage,m.save.campaign!.squad,true);m.autoDeployCampaign();m.start();
    ...
    console.log(stage.id,{won:m.result?.won,core:m.core,time:Math.round(m.time),wave:m.wave.number,kills:m.kills});expect(m.ended).toBe(true);const [region,operation]=stage.id.split('-').map(Number);if(region<=3&&!(region===1&&operation>=9))expect(m.result?.won,`${stage.id} must be clearable by the early-growth starter squad`).toBe(true);if(stage.id==='1-10'||stage.id==='5-10')expect(m.result?.won,`${stage.id} must reject the unequipped starter squad`).toBe(false);
