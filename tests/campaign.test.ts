...
describe('campaign foundations',()=>{
 ...
 it.each(campaignStages)('simulates default squad through $id',async(stage)=>{
   {const save=defaultSave();for(const hero of Object.values(save.heroes)){hero.equipment=[];hero.stars=[1,1,2,2,3,3,4,5][Number(stage.id.split('-')[0])-1];}const m=new BattleModel(save,()=>.5);m.configureCampaign(stage,m.save.campaign!.squad,true);m.autoDeployCampaign();m.start();
