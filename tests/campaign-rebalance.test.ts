...
it('raises area 1 health and attack every operation while introducing tactical enemy roles',()=>{
 ...
 for(let i=1;i<area.length;i++){
  expect(area[i].enemyHp/area[i-1].enemyHp).toBeCloseTo(1.085,2);
  expect(area[i].enemyAttack/area[i-1].enemyAttack).toBeCloseTo(1.06,2);
  expect(area[i].enemySpeed).toBeGreaterThan(area[i-1].enemySpeed);
 }
