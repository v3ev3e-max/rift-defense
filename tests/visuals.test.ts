...
import {enemyMotion,enemyMoveFrameCount} from '../src/game/EnemyMotion';
...
it.each([.75,1,1.5,2])('keeps FX synchronized with combat time at speed %s',speed=>{
 const m=new BattleModel(defaultSave());m.speed=speed;m.emit('shot',0,0,100,100,0xffffff,{duration:.17,visual:'sera-projectile'});
 const expected=.25-(1/12)*battleTimeScale(speed);expect(m.effects[0].life).toBe(.25);m.update(1/12);expect(m.effects[0].life).toBeCloseTo(expected);
 m.paused=true;m.update(.1);expect(m.effects[0].life).toBeCloseTo(expected);
});
...
it('uses the authored regional enemy frame count for regular enemies and bosses',()=>{
 expect(enemyMoveFrameCount(false,true)).toBe(6);
 expect(enemyMoveFrameCount(true,true)).toBe(6);
 expect(enemyMoveFrameCount(false,false)).toBe(6);
 expect(enemyMoveFrameCount(true,false)).toBe(8);
});
