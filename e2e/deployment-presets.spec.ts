...
test('saves, restores and clears three deployment preset slots',async({page})=>{
 ...
 const name=page.locator('[data-preset-name="0"]');await name.fill('보스 공략');await name.blur();await expect(name).toHaveValue('보스 공략');
 await page.evaluate(()=>{const a=(window as any).rift;['yuria','sera','reina','noel','arin'].forEach((id,i)=>a.placeCampaignHero(id,i));});
 ...
 expect(await page.evaluate(()=>Object.fromEntries((window as any).rift.model.units.map((u:any)=>[u.heroId,u.slot])))).toEqual({yuria:0,sera:1,reina:2,noel:3,arin:4});
 ...
 await page.waitForFunction(()=>!!(window as any).rift.game?.scene.getScene('Battle')?.ink);
 await expect(page.locator('.deployment-preset').nth(0)).toHaveClass(/saved/);
 await expect(page.locator('[data-preset-name="0"]')).toHaveValue('보스 공략');
