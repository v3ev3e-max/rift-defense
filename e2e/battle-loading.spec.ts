...
test('전장 이미지가 준비될 때까지 로딩 화면을 유지한다',async({page},testInfo)=>{
  ...
  for(const ratio of coverage){expect(ratio.width).toBeGreaterThan(.95);expect(ratio.height).toBeGreaterThan(.95);}
  ...
  const damageState=await page.evaluate(async()=>{const app=(window as any).rift,scene=app.game.scene.getScene('Battle'),unit=app.model.units[0];app.model.hurtUnit(unit,unit.maxHp*.35);await new Promise(resolve=>setTimeout(resolve,80));return {actual:unit.hp,trail:scene.heroVisualHp[0],flashing:scene.heroHealthHitUntil[0]>scene.visualTime};});
