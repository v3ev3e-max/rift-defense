import {generatedVfxNames,prepareGeneratedVfx} from './GeneratedVfx';
import Phaser from "phaser";
import { assetUrl } from '../utils/assets';
import { heroes, heroById } from "../data/heroes";
import { enemies } from "../data/enemies";
import { HERO_RENDER, poseSize, weaponPoint, dronePoint, fitProjectile } from "./WeaponSockets";
...
export class BattleScene extends Phaser.Scene {
  ...
  preload() {
    for(const name of generatedVfxNames)this.load.image(`vfx-${name}-source`,assetUrl(`/assets/generated/vfx/${name}.webp`));
    ...
    if(this.model.campaign)for(const name of ['pad','ring','warning'])this.load.image(`campaign-${name}`,assetUrl(`/assets/campaign/${name}.png`));
    for (const def of Object.values(enemies)){
      this.load.image(`enemy-${def.id}`, assetUrl(`/assets/generated/enemies/${def.id}.webp`));
      for(let frame=1;frame<=(def.boss?8:6);frame++)this.load.image(`enemy-${def.id}-move-${frame}`,assetUrl(`/assets/generated/enemy-motion/${def.id}/move_${String(frame).padStart(2,'0')}.webp`));
      if(def.ranged)for(let frame=1;frame<=3;frame++)this.load.image(`enemy-${def.id}-fire-${frame}`,assetUrl(`/assets/generated/enemy-fire/${def.id}/frame_${String(frame).padStart(2,'0')}.webp`));
    }
    ...
    for (let frame=1;frame<=4;frame++)
      this.load.image(`enemy-step-${frame}`,assetUrl(`/assets/generated/enemy-steps/step_${String(frame).padStart(2,'0')}.webp`));
    ...
    this.load.image('sniper-round-v2',assetUrl('/assets/generated/sniper-round-v2.png'));
    for(const id of ['yuria','mia','leon','neris','livia'])for(let frame=1;frame<=3;frame++)
      this.load.image(`tank-skill-${id}-${frame}`,assetUrl(`/assets/generated/tank-skills/${id}/frame_${String(frame).padStart(2,'0')}.webp`));
    ...
    for(let frame=1;frame<=3;frame++){
      this.load.image(`enemy-rift-bolt-${frame}`,assetUrl(`/assets/generated/enemy-projectiles/rift_bolt_${String(frame).padStart(2,'0')}.webp`));
      this.load.image(`enemy-ranged-impact-${frame}`,assetUrl(`/assets/generated/enemy-ranged-impact/frame_${String(frame).padStart(2,'0')}.webp`));
    }
  ...
  drawMap() {
    ...
    for(const route of [this.model.map,this.model.campaign?.alternate].filter(Boolean))for (let i = 1; i < route!.path.length; i++) {
      ...
      const a = path[i - 1],
        b = path[i];
      g.lineStyle(76, 0x143c4e);
      g.lineBetween(a.x, a.y, b.x, b.y);
      g.lineStyle(68, 0x9fbcc8);
      g.lineBetween(a.x, a.y, b.x, b.y);
      g.lineStyle(58, 0x617e92);
      g.lineBetween(a.x, a.y, b.x, b.y);
    ...
    for (const [i,s] of slots.entries()){
      ...
      if(this.model.campaign&&i===4)this.add.text(s.x,s.y+35,'FRONT',{fontFamily:'monospace',fontSize:'11px',fontStyle:'bold',color:'#ffd68a',stroke:'#10202d',strokeThickness:3}).setOrigin(.5).setDepth(3);
    ...
    if(this.model.campaign)this.campaignWarnings=Array.from({length:this.model.enemies.length},()=>this.add.image(0,0,'campaign-warning').setDisplaySize(240,240).setDepth(3).setVisible(false));
    ...
    this.add.image(entry.x,ey,'lab-entry').setDisplaySize(82,82).setDepth(2);
    if(this.model.campaign?.alternate){const p=this.model.campaign.alternate.path[0];this.add.image(p.x,p.y,'lab-entry').setDisplaySize(82,82).setDepth(2);}
  ...
  update(_time: number, delta: number) {
    ...
    for(const [ai,a] of m.actions.entries()){if(!a.active||a.age<a.delay)continue;const p=Math.min(1,(a.age-a.delay)/a.duration),u=m.units.find(u=>u.uid===a.owner)??a.source;if(!u)continue;
      ...
      if(a.kind==='sniper'&&!a.hit){const r=30+(1-p)*28;this.actionSprites[ai].setTexture('vfx-reticle').setPosition(a.tx,a.ty).setDisplaySize(r*2,r*2).setRotation((1-p)*.5).setAlpha(1).setVisible(true);const dx=a.tx-a.x,dy=a.ty-a.y;this.aimSprites[ai].setTexture('vfx-bullet').setPosition((a.x+a.tx)/2,(a.y+a.ty)/2).setDisplaySize(Math.hypot(dx,dy),5).setRotation(Math.atan2(dy,dx)).setAlpha(.35).setVisible(true);}
        ...
        if(!a.hit&&a.kind==='meteor'){
          ...
          const x=Phaser.Math.Linear(startX,a.tx,travel),y=Phaser.Math.Linear(startY,a.ty,travel),angle=Math.atan2(a.ty-startY,a.tx-startX)+Math.PI/2;
    ...
    this.campaignWarnings.forEach((sp,i)=>{const e=m.enemies[i];sp.setVisible(e.active&&!!e.strikeAt);if(e.strikeAt)sp.setPosition(e.strikeX!,e.strikeY!).setAlpha(.5+Math.sin(m.time*8)*.15);});
    if(m.campaign){
      ...
      for(const e of m.enemies)if(e.active&&e.strikeAt){g.fillStyle(0xff596f,.12+Math.sin(m.time*9)*.05);g.fillCircle(e.strikeX!,e.strikeY!,100);g.lineStyle(4,0xff7082,.9);g.strokeCircle(e.strikeX!,e.strikeY!,100);}
    }
    ...
    if(selected && !m.paused && !m.choices.length && !m.ended && pointer.x>=0 && pointer.y>=0 && pointer.x<=MAP.width && pointer.y<=MAP.height){
      ...
      if(valid){g.lineStyle(1,color,.3);g.strokeCircle(x,y,m.stats(selected).range);}
    }
    for (let i = 0; i < this.heroSprites.length; i++) {
      ...
      if (!u || u.slot < 0) {
        ...
        const returnPad=u?.returnAt&&u.returnSlot!==undefined?m.map.slots[u.returnSlot]:undefined;
        if(returnPad){const left=Math.max(0,Math.ceil(u!.returnAt!-m.time));defeatCooldown.setVisible(true).setPosition(returnPad.x,returnPad.y).setText(`복귀\n${left}초`).setAlpha(.82+Math.sin(m.time*6)*.18);}
        else defeatCooldown.setVisible(false);
        continue;
      }
    ...
    for (const e of m.enemies) {
      ...
      const bodyCount=boss?8:6,bodyCadence=Math.max(3,Math.min(9,e.speed/9))*(boss?.65:1),bodyFrame=1+Math.floor((this.visualTime*bodyCadence+e.index*.41)%bodyCount);
      ...
      const enemyTexture=firing?`enemy-${e.kind}-fire-${fireFrame}`:`enemy-${e.kind}-move-${bodyFrame}`;if(sp.texture.key!==enemyTexture)sp.setTexture(enemyTexture);
    ...
    for (const i of effectIndices) {if(renderedEffects>=max)break;
      ...
      if(f.visual?.startsWith('tank-skill-')&&fxSpriteIndex<this.fxSprites.length){
        const sp=this.fxSprites[fxSpriteIndex++],id=f.visual.slice('tank-skill-'.length),skillFrame=Math.min(3,Math.floor(progress*3)+1),size=(f.radius??135)*2*(.82+progress*.18);
        sp.setTexture(`tank-skill-${id}-${skillFrame}`).clearTint().setPosition(f.tx,f.ty).setDisplaySize(size,size).setRotation(0).setAlpha(skillFrame===3?1-progress*.45:.98).setDepth(677).setVisible(true);
        continue;
      }
      if(f.visual?.startsWith('tank-guard-hit-')&&fxSpriteIndex<this.fxSprites.length){
        ...
        sp.setTexture(`tank-skill-${id}-${hitFrame}`).clearTint().setPosition(f.tx,f.ty).setDisplaySize(size,size).setRotation(0).setAlpha(1-progress).setDepth(678).setVisible(true);
        continue;
      }
      if(f.visual?.startsWith('hero-defeat-')&&fxSpriteIndex<this.fxSprites.length){
        ...
        sp.setTexture(`hero-${hero}-defeat-${phase+1}`).clearTint().setPosition(f.x,f.y+HERO_RENDER.offsetY)
          .setDisplaySize(HERO_RENDER.size,HERO_RENDER.size).setAngle(0)
      ...
      if(f.visual?.startsWith('transfer-')){
        if(fxSpriteIndex<this.fxSprites.length){
          ...
          if(fxSpriteIndex<this.fxSprites.length){
            ...
            node.setTexture('vfx-impact').setTint(elementColor).setPosition(Phaser.Math.Linear(f.x,f.tx,travel),Phaser.Math.Linear(f.y,f.ty,travel))
          ...
          if(progress>.68&&fxSpriteIndex<this.fxSprites.length){
            ...
            arrival.setTexture('vfx-impact').setTint(elementColor).setPosition(f.tx,f.ty).setDisplaySize(24+arrivalProgress*42,24+arrivalProgress*42)
      ...
      if(f.visual?.startsWith('element-upgrade-')){
        if(fxSpriteIndex<this.fxSprites.length){const sp=this.fxSprites[fxSpriteIndex++],size=(f.radius??62)*2*(.55+progress*.75);sp.setTexture('vfx-zone').setTint(f.color).setPosition(f.tx,f.ty).setDisplaySize(size,size).setRotation(progress*.8).setAlpha(1-progress).setDepth(650).setVisible(true);}continue;
      }
      ...
      if(f.visual==='sniper-round'){
        if(fxSpriteIndex<this.fxSprites.length){
          ...
          sp.setTexture('sniper-round-v2').clearTint().setPosition(Phaser.Math.Linear(f.x,f.tx,travel),Phaser.Math.Linear(f.y,f.ty,travel))
            .setDisplaySize(78,14).setRotation(angle).setAlpha(1-progress*.15).setDepth(672).setVisible(true);
        }
        continue;
      }
      ...
      if(f.visual?.startsWith('reaction-')){
        if(fxSpriteIndex<this.fxSprites.length){const sp=this.fxSprites[fxSpriteIndex++],zone=f.visual==='reaction-corrosion'||f.visual==='reaction-vaporize';sp.setTexture(zone?'vfx-zone':'vfx-impact').setTint(f.color).setPosition(f.tx,f.ty).setDisplaySize(90+progress*80,90+progress*80).setRotation(progress*.6).setAlpha(1-progress).setVisible(true);}continue;
      }
      ...
      if(f.visual?.startsWith('enemy-death-')&&fxSpriteIndex<this.fxSprites.length){
        const sp=this.fxSprites[fxSpriteIndex++],kind=f.visual.slice('enemy-death-'.length),size=enemies[kind]?.boss?126:enemies[kind]?.armor?76:68;
        sp.setTexture(`enemy-${kind}`).setTint(0xd4c8ff).setPosition(f.tx,f.ty-progress*12).setDisplaySize(size*(1+progress*.18),size*(1-progress*.35)).setAngle(progress*18).setAlpha(1-progress).setDepth(675).setVisible(true);continue;
      }
      ...
      if (f.visual && supportedVisual && fxSpriteIndex < this.fxSprites.length) {
        ...
        if (f.visual.endsWith("-projectile")) {
          ...
          const size = (barrier ? 104 : skill ? heroVisuals[visualHero].skill : heroVisuals[visualHero].impact) * (0.65+progress*0.35);
          const angle = progress * (barrier ? 0.16 : skill ? 0.08 : 0.12);
          ...
          sp
            ...
            .setAngle(Phaser.Math.RadToDeg(angle))
            .setDisplaySize(rect.width, rect.height)
            .setAlpha((barrier ? 0.5 : skill ? 0.65 : 0.78) * (1-progress*0.8))
            .setDepth(barrier ? 625 : skill ? 635 : 650);
        }
        continue;
      }
      if(fxSpriteIndex<this.fxSprites.length){
