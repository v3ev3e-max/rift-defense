import { BALANCE, combatRange, REFERENCE_PATH_LENGTH, MELEE_STRIKE_RANGE } from "../data/balance";
import type {CampaignStage} from '../data/campaign';
import {campaignWave,campaignStages} from '../data/campaign';
...
import {combatRoles,formationComplete,formationRole} from '../data/combatRoles';
...
export class BattleModel {
  ...
  configureCampaign(stage:CampaignStage,squad:string[],reserve=false){
    ...
    for(const id of [...new Set(squad)].filter(id=>!!heroById[id]).slice(0,5))this.addUnit(id,1,reserve);
    this.selected=0;this.say('요원을 배치한 뒤 방어 시작을 누르세요.');
  }
  syncCampaignSquad(squad:string[]){
    if(!this.campaign||this.started)return false;
    const ids=[...new Set(squad)].filter(id=>!!heroById[id]).slice(0,5);
  ...
  autoDeployCampaign(){
    ...
    const targets={tank:[4],dealer:[0,1,2],sniper:[5]} as const;
    const used={tank:0,dealer:0,sniper:0};
    for(const u of this.units){u.slot=-1;u.x=-100;u.y=-100;u.moveReadyAt=0;}
    for(const u of this.units){const role=formationRole(u.heroId),slot=targets[role][used[role]++];if(slot===undefined)return false;u.slot=slot;Object.assign(u,this.map.slots[slot]);}
    this.selected=0;this.lastPlacement=undefined;this.say('추천 배치 완료 · FRONT 탱커 / A 저격수 / 후방 딜러 3명');this.refresh();return true;
  }
  hurtUnit(u:Unit,amount:number){
    ...
    if(u.hp<=0){const x=u.x,y=u.y;u.returnSlot=u.slot;u.slot=-1;u.returnAt=this.time+20;this.emit('blast',x,y,x,y,0xffa0b8,{visual:`hero-defeat-${u.heroId}`,duration:.72,sourceHero:u.heroId});this.say(`${heroById[u.heroId].name} 전투 이탈 · 20초 후 복귀`);this.refresh();}
  }
  ...
  stats(u: Unit) {
    ...
    for (const v of this.units) {
      ...
      const auraRange = combatRange(heroById[v.heroId].range, (this.bonuses.range ?? 0) + (this.bonuses.ascend ? 0.15 : 0));
    ...
    return {
      atk:
        h.atk * gradeBasePower[h.grade] * (1+(this.roleUpgrades[combatRoles[u.heroId].kind]??0)*.12+(this.campaign?(this.save.campaign?.research[combatRoles[u.heroId].kind]??0)*.03+(this.save.campaign?.research[h.element]??0)*.03+(this.save.campaign?.fragments[u.heroId]??0)*.005:0)) *
        (1 + 0.04 * (lv - 1)) *
        Math.pow(gradeStarGrowth[h.grade], u.star - 1) *
        (1 + e.attack) *
        (1 +
      ...
      speed:
        ...
        (u.stunned > 0 ? 0.55 : 1),
      range: (()=>{const bonus=(this.bonuses.range??0)+(this.bonuses.ascend?0.15:0),native=combatRange(h.range);return combatRoles[u.heroId].kind==='melee'&&formationRole(u.heroId)!=='tank'?Math.max(MELEE_STRIKE_RANGE,native)+Math.min(CELL,combatRange(h.range,bonus)-native):combatRange(h.range,bonus);})(),
      crit: Math.min(
        0.85,
        0.06 +
          e.crit +
  ...
  spawn(kind: string) {
    ...
    if(this.campaign){
      const index=campaignStages.indexOf(this.campaign);const hpScale=.55+index*.18;
      e.hp*=hpScale;e.maxHp=e.hp;e.speed=def.speed*.43;
  ...
  damage(e: Enemy, amount: number, owner?: Unit, secondary = false, critical=false) {
    ...
    const real = Math.min(e.hp, Math.max(0, amount * phase * (1 + e.vulnerability)));
    e.hp -= real;
  ...
  update(deltaSeconds: number) {
    ...
    // FX use real elapsed time: x2 accelerates combat, not the 12 FPS art.
    for (const fx of this.effects) fx.life = Math.max(0, fx.life - Math.min(deltaSeconds, 0.25));
  ...
  step(dt: number) {
    ...
    if(this.campaign)for(const u of this.units){
      if(u.returnAt&&this.time>=u.returnAt){const slot=u.returnSlot!;u.hp=u.maxHp;u.returnAt=undefined;if(!this.units.some(v=>v!==u&&v.slot===slot)){u.slot=slot;Object.assign(u,this.map.slots[slot]);}this.say(`${heroById[u.heroId].name} 복귀${u.slot<0?' · 대기 요원을 선택해 배치하세요.':''}`);}
