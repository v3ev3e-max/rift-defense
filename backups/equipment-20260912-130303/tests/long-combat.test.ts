...
it('ten minutes of mixed combat keeps every transient pool bounded',()=>{
 ...
 expect(m.time).toBeCloseTo(600,5);expect(m.actions).toHaveLength(96);expect(m.zones).toHaveLength(12);expect(m.effects).toHaveLength(64);expect(m.damageNumbers).toHaveLength(16);expect(m.dotLabels.size).toBeLessThanOrEqual(48);expect(m.records).toHaveLength(12);
