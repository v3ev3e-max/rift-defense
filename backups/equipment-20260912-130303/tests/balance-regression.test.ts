...
import {combatRange,MELEE_STRIKE_RANGE} from '../src/data/balance';
...
it.each(heroes)('$id retains a bounded range after range upgrades',h=>{
 ...
 const expected=combatRoles[h.id].kind==='melee'&&formationRole(h.id)!=='tank'?Math.max(MELEE_STRIKE_RANGE,combatRange(h.range)):combatRange(h.range);
