...
function collect(dir) {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (mime[extname(path)] && (!(path.includes('/assets/generated/')&&extname(path)==='.png') || /\/attack\/frame_\d+\.png$/.test(path))) assets[path.slice('public'.length)] = `data:${mime[extname(path)]};base64,${readFileSync(path).toString('base64')}`;
  }
}
